...
from rl.Dream.utils.utils import export_policy_as_jit
import torch

from rl.Dream.algorithms import PPO
from rl.Dream.modules import ActorCritic, ActorCriticRecurrent
from rl.Dream.env import VecEnv


from rl.Dream.modules.actor_critic import CeEncoder


class DreamPolicyRunner:
    ...
    def __init__(self,
        ...
        num_decoder_input = self.env.num_obs + self.proprio_adapt_output

        self.encoder_decoder = CeEncoder(num_decoder_input, self.encoder_mlp, self.decoder_mlp)
        ...
        self.alg.init_storage(self.env.num_envs, self.num_steps_per_env, [self.env.num_obs], [self.env.num_privileged_obs], [self.env.num_actions], [self.rma_cfg["Hist_info_dim"]])
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            mean_value_loss, mean_surrogate_loss, mean_vel_loss, mean_vae_loss, mean_kl_loss = self.alg.update()
            stop = time.time()
    ...
    def log(self, locs, width=80, pad=35):
        ...
        self.writer.add_scalar('Vel/vae_loss',  locs['mean_vae_loss'], locs['it'])
        self.writer.add_scalar('Vel/kl_loss',   locs['mean_kl_loss'],  locs['it'])
    ...
    def save(self, path, infos=None):
        torch.save({
            ...
            "encoder_decoder_state_dict": self.alg.encoder_decoder.state_dict(),
    ...
    def load(self, path, load_optimizer=True):
        ...
        self.alg.encoder_decoder.load_state_dict(loaded_dict["encoder_decoder_state_dict"])
        ...
        if self.rma_cfg['export_policy']:
            ...
            export_policy_as_jit(self.alg.actor_critic.encoder_decoder, jit_save_path, 'encoder.pt')
            print(f"actor: {self.alg.actor_critic.actor}")
            print(f"encoder: {self.alg.actor_critic.encoder_decoder}")
